...
uniform float K <
    ui_label = "Capacitance K";
    ui_tooltip = "Color.C*K";
    ui_type = "slider";
    ui_min = 0.0;
    ui_max = 15000.0;
    ui_step = 1.0;
> = 12000.0;
uniform float C_r < ui_label = "Capacitance R (pF)"; ui_min = 1.0; ui_max = 100.0; ui_step = 1.0; > = 20.0;
uniform float C_g < ui_label = "Capacitance G (pF)"; ui_min = 1.0; ui_max = 100.0; ui_step = 1.0; > = 20.0;
uniform float C_b < ui_label = "Capacitance B (pF)"; ui_min = 1.0; ui_max = 100.0; ui_step = 1.0; > = 20.0;

uniform float mask_step_mm < ui_label = "Shadow mask pitch (mm)"; ui_min = 0.1; ui_max = 1.0; ui_step = 0.01; > = 0.28;

uniform int rc_effect_level < ui_label = "RC Lookahead (pixels)"; ui_min = 1; ui_max = BUFFER_WIDTH; ui_step = 1; > = 128;
...
float3 simulateRCResponse(float2 baseCoord, sampler2D inputSampler, float pixel_dt, float3 tau, int N)
{
    float3 I_real = tex2D(inputSampler, baseCoord - float2(N, 0) * BUFFER_RCP_WIDTH).rgb;
    // Начинаем с "далёкого" пикселя, чтобы система пришла в равновесие

    for (int i = -N + 1; i <= 0; i++)
    {
        float2 coord = baseCoord + float2(i, 0) * BUFFER_RCP_WIDTH;
        float3 I_target = tex2D(inputSampler, coord).rgb;
        float3 alpha = 1.0 - exp(-pixel_dt / max(tau, 1e-12));
        I_real = I_real + (I_target - I_real) * alpha;
    }
    return I_real;
}
...
// === ПРОХОД RC: МОДЕЛИРОВАНИЕ ОГРАНИЧЕНИЯ СКОРОСТИ КАНАЛОВ ===
float4 PassRC_SimulateBeamResponse(float4 vpos : SV_Position, float2 texcoord : TexCoord) : SV_Target
{
    const float R_out = 100.0; // Ом
  //  const float pF_to_F = 1e-12;
	//float3 tau = R_out * float3(C_r, C_g, C_b) * pF_to_F; // секунды
	float pixel_duration = 1.0 / (FPS * BUFFER_HEIGHT); // секунды на пиксель (приближение)
	
    float2 texsize = float2(BUFFER_WIDTH, BUFFER_HEIGHT);
    float pixel_dt = 1.0 / (FPS * texsize.y); // секунд на пиксель

    float3 tau = R_out * float3(C_r*K, C_g*K, C_b*K) * 1e-12; // сек

    float3 rc_result = simulateRCResponse(texcoord, AccumBSampler, pixel_dt, tau, rc_effect_level);

    return float4(rc_result, 1.0);
}

float4 PassRC_Copy(float4 vpos : SV_Position, float2 texcoord : TexCoord) : SV_Target
{
    return tex2D(AccumASampler, texcoord);
}
...
technique ShadowCRT
{
    ...
    {
        VertexShader = PostProcessVS;
        PixelShader = PassRC_SimulateBeamResponse;
        RenderTarget = AccumA;
    }
